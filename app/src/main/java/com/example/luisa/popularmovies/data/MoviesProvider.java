 ...
 */
public class MoviesProvider extends AbstractContentProvider {

    static final int MOVIE_WITH_ID = 101;

    @Override
    public AbstractContract getContract() {
        return new MoviesContract();
    }
    ...
    protected UriMatcher onBuildUriMatcher(UriMatcher matcher) {
        ...
        matcher.addURI(getContract().getContentAuthority(), MoviesContract.BASE_PATH + "/*", MOVIE_WITH_ID);
        return matcher;
    }
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case MOVIE_WITH_ID:
                return new MoviesContract().getBaseEntry().CONTENT_ITEM_TYPE;
            default:
